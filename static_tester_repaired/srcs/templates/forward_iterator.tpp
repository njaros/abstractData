...
void forward_iteratorSet(T)
{
	...
	(void)*a;
	static_assert(std::is_same<decltype(*a), const T&>::value, "wrong return type for operator*()");
//	a.operator->();
//	static_assert(std::is_same<decltype(a.operator->()), T*>::value, "wrong return type for operator->()");
	...
	static_assert(std::is_convertible<decltype(a++), const X&>::value, "wrong return type for operator++(int)");
	(void)*a++;
	static_assert(std::is_same<decltype(*a++), const T&>::value, "wrong return type for *it++");
