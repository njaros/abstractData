...
void unordered_multiset_tests(const std::string& currentPath)
{
	...
	typedef unordered_multiset<std::string> S;
	typedef unordered_multiset<int> I;
	typedef unordered_multiset<float> F;
	typedef unordered_multiset<double> D;
	typedef unordered_multiset<int*> P;

	multiset<std::string> mapS;
	mapS.insert("hello");
	mapS.insert("John");
	mapS.insert("coucou");
	mapS.insert("pastis");
...

	{
		...
		const multiset<float> m(f.begin(), f.end());
...

	{
		...
		for (int i = -100; i < 100; ++i)
		{
			...
			{
				tmpCit = cit;
				while (++tmpCit != m.end() && *tmpCit == *cit)
					outfile << *tmpCit << " found\n";
				outfile << *it << '\n';
			}
		}
