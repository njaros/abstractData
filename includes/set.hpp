...
// CONSTRUCTORS / DESTRUCTOR / OPERATOR=
        ...
        set(InputIterator first, InputIterator last) : _root(0), _size(0)
        {
			_leaf = doALeaf(value_type(), _allocNode, _alloc);
            while (first != last) {
				insert(*first);
        ...
        set(const set &other) : _root(0), _size(0)	{
			_leaf = doALeaf(value_type(), _allocNode, _alloc);
            *this = other;
        }
...
// MODIFIERS
        ...
        {
			ft::vector<value_type>	lst;
			while (first != last) {
				lst.push_back(*first);
				++first;
			}
			for (typename ft::vector<value_type>::iterator begin = lst.begin(); begin != lst.end(); ++begin) {
				erase(*begin);
			}
        }
        void                        swap(set &other) {
